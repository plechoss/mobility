...
n_rows = st.number_input(f'Pick a number of waypoints to be loaded (max {str(data_length)}):', min_value=0, max_value=data_length, value=10000)
...
st.code('''data = waypointsdataframe.WaypointsDataFrame(data)''', language='python')

data = waypointsdataframe.WaypointsDataFrame(data)
...
selected_timestamps = st.multiselect('Which legs do you want to see?', starting_timestamps, default=starting_timestamps[16:21], format_func=get_name)
#filter legs df
